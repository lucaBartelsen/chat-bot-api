# app/api/examples.py - Fixed to properly handle async operations
...
async def create_response_example(
    ...
    try:
        ...
        # FIXED: Get complete example with responses using eager loading
        query = (
            select(ResponseExample)
            .options(selectinload(ResponseExample.responses))  # Eager load responses
...
async def get_response_examples(
    ...
    # FIXED: Base query with eager loading
    query = (
        select(ResponseExample)
        .options(selectinload(ResponseExample.responses))  # Eager load responses
...
async def get_response_example(
    ...
    # FIXED: Get example with eager loading
    query = (
        select(ResponseExample)
        .options(selectinload(ResponseExample.responses))  # Eager load responses
        .where(
